...
fn allocateMemory(py: Python<'_>, size: Py<PyAny>) -> Py<PyAny> {
    let mut buffer = (allocate(py, (size).clone1(py).into_py_any(py).unwrap()))
        .clone1(py)
        .into_py_any(py)
        .unwrap();
...
fn optimizeMe(py: Python<'_>) -> Py<PyAny> {
    let mut sum = (3).clone1(py).into_py_any(py).unwrap();
...
fn processStream(py: Python<'_>, streamSize: Py<PyAny>) -> Py<PyAny> {
    ...
    let mut streamPtr = (allocate(py, (streamSize).clone1(py).into_py_any(py).unwrap()))
        .clone1(py)
        .into_py_any(py)
        .unwrap();
    ...
    while (((i).clone1(py).into_py_any(py).unwrap())
        ...
        < (((streamSize).clone1(py).into_py_any(py).unwrap())
            ...
            .unwrap())
    {
        let mut blockPtr = ((((streamPtr).clone1(py).into_py_any(py).unwrap())
            .extract::<Buffer>(py)
            .unwrap())
        .borrow(
            py,
