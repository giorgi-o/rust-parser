...
fn allocateMemory(py: Python<'_>, size: Py<PyAny>) -> Py<PyAny> {
    let buffer = (allocate(py, (size).clone1(py).into_py_any(py).unwrap()))
        .clone1(py)
        .into_py_any(py)
        .unwrap();
...
fn optimizeMe(py: Python<'_>) -> Py<PyAny> {
    let sum = (3).clone1(py).into_py_any(py).unwrap();
    allocate(py, (sum).clone1(py).into_py_any(py).unwrap());
    return py.None();
}

#[pyfunction]
fn processStream(py: Python<'_>, streamSize: Py<PyAny>) -> Py<PyAny> {
    let blocksize = (10).clone1(py).into_py_any(py).unwrap();
    let streamPtr = (allocate(py, (streamSize).clone1(py).into_py_any(py).unwrap()))
        .clone1(py)
        .into_py_any(py)
        .unwrap();
    ...
    while (((i).clone1(py).into_py_any(py).unwrap())
        .extract::<usize>(py)
        .unwrap())
        < (((streamSize).clone1(py).into_py_any(py).unwrap())
            .extract::<usize>(py)
            .unwrap())
    {
        let blockPtr = ((((streamPtr).clone1(py).into_py_any(py).unwrap())
            .extract::<Buffer>(py)
            .unwrap())
        .borrow(
            py,
            (blocksize).clone1(py).into_py_any(py).unwrap(),
            (i).clone1(py).into_py_any(py).unwrap(),
        ))
        .clone1(py)
        .into_py_any(py)
        .unwrap();
        blocks
            .call_method(
                py,
                "append",
                ((blockPtr).clone1(py).into_py_any(py).unwrap(),),
                None,
            )
            .unwrap();
        i = ((((i).clone1(py).into_py_any(py).unwrap())
            .extract::<usize>(py)
            .unwrap())
            + (((blocksize).clone1(py).into_py_any(py).unwrap())
                .extract::<usize>(py)
