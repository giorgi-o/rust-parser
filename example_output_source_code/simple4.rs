...
#[pyfunction]
fn foo(py: Python<'_>, big: Py<PyAny>) -> Py<PyAny> {
    let mut count = (10).clone1(py).into_py_any(py).unwrap();
    let mut arr = (PyList::new(py, Vec::<Buffer>::new()).unwrap().unbind())
        .clone1(py)
        .into_py_any(py)
        .unwrap();
    ...
    while (((i).clone1(py).into_py_any(py).unwrap())
        .extract::<usize>(py)
        .unwrap())
        < (((count).clone1(py).into_py_any(py).unwrap())
            .extract::<usize>(py)
            .unwrap())
    {
        let mut buffer = (allocate(py, (1024).clone1(py).into_py_any(py).unwrap()))
            .clone1(py)
            .into_py_any(py)
            .unwrap();
        arr.call_method(
            py,
            "append",
            ((buffer).clone1(py).into_py_any(py).unwrap(),),
            None,
        )
        .unwrap();
        i = ((((i).clone1(py).into_py_any(py).unwrap())
            .extract::<usize>(py)
            .unwrap())
            + (((1).clone1(py).into_py_any(py).unwrap())
                .extract::<usize>(py)
                .unwrap()))
        .clone1(py)
        .into_py_any(py)
    ...
    return (arr).clone1(py).into_py_any(py).unwrap();
    return py.None();
}
